...
def md_get_is_private_catalog():
  return asbool(config.get('ckan.odczdataset.private_catalog', False))
...
class ODCZDatasetFormPlugin(p.SingletonPlugin, tk.DefaultDatasetForm):
  ...
  def set_SHA(self, context, resource):
    ...
    if resource['url_type']:
        ...
        # `hash` field exists in standard scheme but is not being used by standard code.
        # https://github.com/ckan/ckan/issues/3607
        x.hash = sha.hexdigest()
  ...
  def get_helpers(self):
    return {
      ...
      'md_get_is_private_catalog': md_get_is_private_catalog,
    }
...
#      'ruian_type': [tk.get_converter('convert_from_tags')('ruian_types'),
  ...
  def my_package_type(self, data, package_type):
    return package_type
